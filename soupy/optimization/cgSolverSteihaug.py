...
def CGSolverSteihaug_ParameterList():
    """
    Generate a :code:hippylib.`ParameterList` for :code:`CGSolverSteihaug`.
    ...
    parameters["print_level"] = [0, "verbosity level: -1 --> no output on screen; 0 --> only final residual at convergence or reason for non-convergence"]
...
class CGSolverSteihaug:
    ...
    def __init__(self, parameters=CGSolverSteihaug_ParameterList()):
        ...
        self.A = None
        self.B_solver = IdentityOperator()
        self.B_op = IdentityOperator()
    ...
    def set_preconditioner(self, B_solver):
        ...
        """
        if B_solver is not None:
            assert hasattr(B_solver, "solve"), "Preconditioner needs to have solve method"
            self.B_solver = B_solver

    def set_TR(self, radius, B_op=None):
    ...
    def solve(self,x,b):
        ...
        # Initialize the supporting vectors based on the initial guess 
        self.r = x.copy()
        self.z = x.copy()
        self.Ad = x.copy()
        self.d = x.copy()
