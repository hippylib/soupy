...
This implements the Gaussian mixture Taylor approximation for mean + variance from:
...
For the linear case, each component Q_lin,i is Gaussian N(mu_i, sigma_i^2).
The mixture mean and variance are:
    E[Q_mix] = sum_i w_i * mu_i
    Var[Q_mix] = sum_i w_i * (mu_i^2 + sigma_i^2) - E[Q_mix]^2

Risk measure: Mean + beta * Variance
"""
...
def gaussian_mixture_mean_variance(means, stds, weights):
    ...
    For Q_mix ~ sum_i w_i N(mu_i, sigma_i^2), computes:
        E[Q_mix] = sum_i w_i * mu_i
        E[Q_mix^2] = sum_i w_i * (mu_i^2 + sigma_i^2)
        Var[Q_mix] = E[Q_mix^2] - E[Q_mix]^2

    Args:
        means: Array of component means (mu_i = Q(m_bar_i))
...
class _TaylorMixtureLinearLegacy:
    ...
    def __init__(self, settings, model, prior, penalization, tol=1e-9):
        ...
        self.beta = settings["beta"]  # Variance weight
        ...
        # For gradient computation - store linearization info for each component
        self.x_components = []  # State at each component mean
        self.y_components = []  # Adjoint at each component mean
        self.g_components = []  # Gradient at each component mean
    ...
    def objective(self):
        ...
        # Compute mean and variance of the Gaussian mixture
        self.mixture_mean, self.mixture_var = gaussian_mixture_mean_variance(
            self.component_means,
            self.component_stds,
        ...
        if self.verbose and self.mpi_rank == 0:
            print(f"  [Mixture Linear] N_mix={self.N_mix}, Mean={self.mixture_mean:.4e}, Var={self.mixture_var:.4e}")
            for i in range(self.N_mix):
    ...
    def costValue(self, z):
        ...
        # Reset direction computation for new control
        self.direction_computed = False
    ...
    def costGradient(self, z):
        ...
        The gradient of Mean + beta * Variance w.r.t. control z is:
            d/dz [E[Q] + beta * Var[Q]]
          = d/dz [sum_i w_i mu_i] + beta * d/dz [sum_i w_i (mu_i^2 + sigma_i^2) - (sum_i w_i mu_i)^2]

        For simplicity, we compute the weighted average of component gradients.
        """
        self.grad_ncalls += 1
        self.z.zero()
        self.z.axpy(1.0, z)

        # Ensure direction is computed (do NOT reset here - we need the cached components)
        ...
        # d(Mean)/dz = sum_i w_i * d(mu_i)/dz
        # For variance term, we use a simplified approximation
        for i in range(self.N_mix):
            m_i = self.m_bar_i[i]
            ...
            # Gradient of Q_i w.r.t. control
            dz_i = self.model.generate_vector(CONTROL)
            self.pde.evalGradientControl(x_all_i, dz_i)

            # Weight by component weight
            # For mean: w_i * d(mu_i)/dz
            # For variance: more complex, but we approximate with weighted mean gradient
            w_i = self.component_weights[i]
            mu_i = self.component_means[i]

            # Gradient contribution: w_i * (1 + 2*beta*(mu_i - E[Q])) * d(mu_i)/dz
            mean_weight = 1.0 + 2.0 * self.beta * (mu_i - self.mixture_mean)
            dz.axpy(w_i * mean_weight, dz_i)
...
class TaylorMixtureLinearControlCostFunctional(ControlCostFunctional):
    ...
    def mixture_mean(self):
        """Return the mixture mean value."""
        return self._legacy.mixture_mean

    @property
    def mixture_var(self):
        """Return the mixture variance value."""
