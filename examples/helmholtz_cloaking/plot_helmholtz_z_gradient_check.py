"""Directional finite-difference check for z-gradient on Helmholtz cloaking."""
...
def main():
    parser = argparse.ArgumentParser(description="z-gradient FD check for Helmholtz cloaking")
    ...
    parser.add_argument("--out", type=str, default="helmholtz_z_gradient_fd_error.png")
    ...
    _, _, control_model, prior, penalty, _ = setup_helmholtz_cloaking_problem(
        nx=args.nx,
    ...
    plt.xlabel("epsilon")
    plt.ylabel("|FD - g.dz|")
    plt.title("Helmholtz z-gradient FD error")
    ...
    plt.title("Helmholtz z-gradient relative FD error")
    plt.grid(True, which="both", alpha=0.25)
    plt.legend()
    plt.tight_layout()
