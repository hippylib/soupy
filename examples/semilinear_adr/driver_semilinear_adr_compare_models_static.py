"""Compare static approximation errors on semilinear ADR with z = 0.
...
def evaluate_linear_cvar(cost_functional) -> float:
...
def evaluate_quadratic_cvar(cost_functional, beta: float) -> float:
...
def evaluate_mixture_model(model_name: str, control_model, prior, args, gt_stats: Dict[str, float]) -> Dict[str, float]:
    start = time.time()

    mean_cost = make_meanvar_mixture_cost(model_name, control_model, prior, args, beta=0.0)
    z0_mean = mean_cost.generate_vector(soupy.CONTROL)
    z0_mean.zero()
    z0_mean.apply("")
    mean_val = float(mean_cost.cost(z0_mean, order=0))

    var_cost = make_meanvar_mixture_cost(model_name, control_model, prior, args, beta=1.0)
    z0_var = var_cost.generate_vector(soupy.CONTROL)
    z0_var.zero()
    z0_var.apply("")
    mean_plus_var = float(var_cost.cost(z0_var, order=0))
    ...
    if "quadratic" in model_name:
        cvar_val = evaluate_quadratic_cvar(cvar_cost, args.cvar_beta)
    else:
        cvar_val = evaluate_linear_cvar(cvar_cost)
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare static approximation errors for semilinear ADR at z = 0"
    )
    ...
    parser.add_argument("--mc-trials", type=int, default=200) # Here DC used 1000
    ...
    parser.add_argument("--quadratic-cvar-n-mc", type=int, default=1000) # Here DC used 10^5
    parser.add_argument("--nx", type=int, default=32, help="Optional mesh override for debugging") # DC used 128
    parser.add_argument("--ny", type=int, default=32, help="Optional mesh override for debugging") # DC used 128
    ...
    if rank == 0:
        print("=" * 90)
        print("Semilinear ADR Static Approximation Error Comparison (L2 QoI, z = 0)")
        print("=" * 90)
    ...
    for n_mix in args.mixture_sizes:
        ...
        for model_name in MIXTURE_MODEL_ORDER:
            row = evaluate_mixture_model(model_name, control_model, prior, args, gt_stats)
            rows.append(row)
            if rank == 0:
                print_result_line(row)
