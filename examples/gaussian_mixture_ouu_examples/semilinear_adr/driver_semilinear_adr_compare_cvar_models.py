...
def plot_curves(results, save_dir):
    ...
    ax_obj.set_ylabel("Model CVaR Objective")
    ax_obj.set_title("CVaR Objective Value per Iteration")
...
def save_cvar_tail_parameter_solution_plots(results, saa_model_name, control_model, prior, Vh, save_dir, sample_size, cvar_value, sample_count=3, seed=1):
    ...
    V_parameter_scalar = dl.FunctionSpace(V_parameter.mesh(), "CG", 1)
    V_state_scalar = dl.FunctionSpace(V_state.mesh(), "CG", 1)
    control_np = results[saa_model_name]["control_opt_np"]
    noise = dl.Vector(V_parameter.mesh().mpi_comm())
    prior.init_vector(noise, "noise")
    rng = hp.Random(seed=seed)
    tail_samples = []
    ...
    for i, (qoi, m, state_vec) in enumerate(tail_samples):
        ...
        for ax, fun, title in zip(
            axes[i],
            [m_fun, state_fun],
            [f"tail sample {i + 1}\nQoI={qoi:.2e} > CVaR={cvar_value:.2e}", "solution at tail sample"],
        ):
            artist = plot_on_axes(fun, ax)
            ax.set_xlabel("x")
            ax.set_ylabel("y")
    ...
    plt.close(fig)
    np.savez(
        os.path.join(save_dir, f"{saa_model_name}_tail_parameter_solution_samples_data.npz"),
        seed=seed,
...
def save_optimal_pde_solution_plot(results, control_model, prior, Vh, save_dir):
    ...
    np.savez(
        ...
        model_order=np.array([m for m, _ in payload]),
        **{f"control_np__{model_name}": results[model_name]["control_opt_np"] for model_name in MODEL_ORDER},
    )
...
def main():
    ...
    parser.add_argument("--maxiter-saa", type=int, default=1200)
