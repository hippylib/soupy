"""Directional finite-difference checks for CVaR Taylor-model gradients on semilinear ADR.

This mirrors the style of ``soupy/test/plot_cvar_z_gradient_check.py``, adapted to the
semilinear ADR control problem used by
``driver_semilinear_adr_compare_cvar_models.py``. For each approximation, this script
computes
...
where g(z, t) is the returned gradient. For augmented CVaR models (quadratic and
mixture-quadratic) we also check the pure ``t`` direction separately.
...
zᵢ = sin(2π·xᵢ)·sin(2π·yᵢ) at each well center, with the auxiliary scalar t fixed
at 0. All model hyperparameters (cvar_beta, N_tr, N_mix, quadratic_cvar_n_mc,
penalty) match the defaults used in ``driver_semilinear_adr_compare_cvar_models.py``.
...
def build_problem(args):
    """Set up the semilinear ADR control problem exactly as in the CVaR comparison driver."""
    ...
    return control_model, prior, penalty, control_parameters
...
def save_combined_error_figure(results, epsilons, save_dir, base_name, abs_ylabel, rel_ylabel,
    ...
    """Plot |FD - g.direction| (left) and its relative version (right) side by side.

    The legend (including O(epsilon) reference line, if any) is drawn only once,
    to the right of both subplots.
    """
    fig, (ax_abs, ax_rel) = plt.subplots(1, 2, figsize=(14, 5.5))
    for r in results:
        label_abs = f"{r['label']} (slope~{r['slope']:.2f})" if "slope" in r else r["label"]
        ax_abs.loglog(epsilons, r["abs_err"], r["style"], label=label_abs)
...
def main():
    parser = argparse.ArgumentParser(
        description="Check z- and t-gradient accuracy for CVaR Taylor approximations on semilinear ADR"
    )
    # CVaR / surrogate hyperparameters: match driver_semilinear_adr_compare_cvar_models.py defaults
    ...
    parser.add_argument("--quadratic-cvar-n-mc", type=int, default=10000)
    parser.add_argument("--epsilon", type=float, default=1e-4, help="Smoothing epsilon for quadratic/mixture-quadratic CVaR")
    parser.add_argument("--penalty", type=float, default=0.0)

    # Semilinear ADR problem settings: match driver_semilinear_adr_compare_cvar_models.py defaults
    ...
    control_model, prior, penalty, control_parameters = build_problem(args)
    ...
    # driver_semilinear_adr_compare_models_static.py, with t fixed at 0.
    ...
    for key, label, style, cost in model_specs:
        ...
        if key in {"quadratic", "mixture-quadratic-kle", "mixture-quadratic-hep"}:
            ...
            t_results.append(
                {"key": key, "label": label, "style": style, "true": t_true,
                 "fd": t_fd, "abs_err": t_abs_err, "rel_err": t_rel_err, "slope": t_slope}
            )
    ...
    out_path, data_path = save_combined_error_figure(
        ...
        epsilons,
        args.save_dir,
        "semilinear_adr_cvar_z_gradient_fd_error",
        abs_ylabel="|FD - g·dz|",
    ...
    if t_results:
        out_path_t, data_path_t = save_combined_error_figure(
            t_results,
            epsilons,
            args.save_dir,
            "semilinear_adr_cvar_t_gradient_fd_error",
            abs_ylabel="|FD - g·dt|",
            rel_ylabel="|FD - g·dt| / max(|g·dt|, 1e-14)",
            title_abs="Absolute t-gradient FD error",
            title_rel="Relative t-gradient FD error",
            fit_start=args.fit_start,
        )
        print(f"Saved plot to: {out_path_t}")
        print(f"Saved data to: {data_path_t}")
