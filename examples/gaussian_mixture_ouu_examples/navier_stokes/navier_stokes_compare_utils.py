...
def _save_plot_data(save_dir, name, **data):
    """Archive the numeric data backing a saved figure next to the image."""
    np.savez(os.path.join(save_dir, f"{name}_data.npz"), **data)
...
def save_parameter_sample_plots(prior, Vh, save_dir, sample_count=3, seed=11):
    ...
    for i, ax in enumerate(axes):
        ...
        ax.set_xlabel("x")
        ax.set_ylabel("y")
        plt.colorbar(artist, ax=ax, fraction=0.046, pad=0.1)
    fig.tight_layout()
    fig.savefig(os.path.join(save_dir, "parameter_samples.png"), dpi=180)
    plt.close(fig)
    _save_plot_data(save_dir, "parameter_samples", seed=seed, parameter_dofs=np.array(parameter_dofs))
...
def plot_boundary_control_profiles(control_np, control_bases, axes, model_name=None):
    ...
    for ax, (side, arclength, radial_velocity) in zip(axes, profiles):
        ...
        if model_name is None:
            ax.set_title(f"{side} radial velocity")
        else:
            ax.set_title(f"{model_name}\n{side} radial velocity")
...
def save_saa_parameter_solution_sample_plots(results, saa_model_name, control_model, prior, Vh, save_dir, sample_count=3, seed=11):
    ...
    _save_plot_data(
        save_dir,
        f"{saa_model_name}_parameter_solution_samples",
        seed=seed,
...
def save_solution_vs_linear_initial_plots(results, model_order, linear_initial_control_np, control_model, prior, Vh, save_dir):
    ...
    for i, model_name in enumerate(model_order):
        control_np = optimal_control_np(results[model_name])
        ...
        for ax, fun, title in zip(
            ...
            [f"{model_name}\nv_x(z*)", f"{model_name}\nv_y(z*)", "linear initial v_x", "linear initial v_y"],
        ):
            artist = plot_on_axes(fun, ax)
            ax.set_xlabel("x")
            ax.set_ylabel("y")
    ...
    _save_plot_data(
        save_dir,
        "optimal_vs_linear_initial_solutions",
...
def save_cvar_tail_parameter_solution_plots(results, saa_model_name, control_model, prior, Vh, save_dir, sample_size, cvar_value, sample_count=3, seed=1):
    ...
    for i, (qoi, m, state_vec) in enumerate(tail_samples):
        ...
        for ax, fun, title in zip(
            axes[i],
            [m_fun, state_fun],
            [f"tail sample {i + 1}\nQoI={qoi:.2e} > CVaR={cvar_value:.2e}", "|v| at tail sample"],
        ):
            artist = plot_on_axes(fun, ax)
            ax.set_xlabel("x")
            ax.set_ylabel("y")
    ...
    _save_plot_data(
        save_dir,
        f"{saa_model_name}_tail_parameter_solution_samples",
        seed=seed,
        cvar_value=float(cvar_value),
        control_np=np.array(control_np, copy=True),
        tail_qoi_values=np.array([float(qoi) for qoi, _, _ in tail_samples]),
    )
...
def save_optimal_field_plots(results, model_order, control_model, prior, Vh, save_dir):
    with plt.rc_context(LARGE_FONT_RC):
        ...
        for model_name in model_order:
            ...
            axes[2].set_xlabel("x")
            axes[2].set_ylabel("y")
            plt.colorbar(artist, ax=axes[2], fraction=0.046, pad=0.1)
            fig.tight_layout()
            fig.savefig(os.path.join(save_dir, f"{model_name}_optimal_fields.png"), dpi=180)
            plt.close(fig)
            _save_plot_data(save_dir, f"{model_name}_optimal_fields", control_np=np.array(control_np, copy=True))
        ...
        _save_plot_data(
            save_dir,
            "optimal_fields_overview",
            model_order=np.array([name for name, _, _ in overview_payload]),
            **{f"control_np__{name}": arr for name, arr, _ in overview_payload},
        )
...
def save_optimal_state_component_plots(results, model_order, control_model, prior, Vh, save_dir):
    ...
    for i, (model_name, v_x, v_y) in enumerate(payload):
        for ax, fun, title in zip(axes[i], [v_x, v_y], [f"{model_name}\nv_x(z*)", f"{model_name}\nv_y(z*)"]):
            artist = plot_on_axes(fun, ax)
            ax.set_title(title)
            ax.set_xlabel("x")
            ax.set_ylabel("y")
    ...
    _save_plot_data(
        save_dir,
        "optimal_state_components",
        model_order=np.array([name for name, _, _ in payload]),
        **{f"control_np__{name}": np.array(optimal_control_np(results[name]), copy=True) for name in model_order},
    )
...
def save_optimal_pde_solution_plot(results, model_order, control_model, prior, Vh, save_dir):
    ...
    _save_plot_data(
        save_dir,
        "optimal_pde_solutions",
        model_order=np.array([name for name, _ in payload]),
        **{f"control_np__{name}": np.array(optimal_control_np(results[name]), copy=True) for name in model_order},
    )
